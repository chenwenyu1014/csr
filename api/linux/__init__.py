...
1. constraint_validation  - 数据源约束验证接口 (POST /api/v1/validation/data-source)
...
6. allocation             - 文件分配接口 (POST /api/v1/datasource/allocate)
7. postprocessing         - 后处理接口 (POST /api/v1/document/clean)
8. template               - 模板处理接口 (POST /api/v1/template/process)
9. file_validation        - 文件校验接口 (POST /api/v1/validation/file-validate)
10. manifest              - 清单生成接口 (POST /api/v1/files/manifest)
