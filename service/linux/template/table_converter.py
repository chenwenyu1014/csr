...
class TableConverter:
    ...
    def convert_table(
        ...
        try:
            ...
            # ========== 3. pdf2image 转 PNG（保存到run_dir/img）==========
        ...
        for attempt in range(max_retries):
            try:
                ...
                if result.returncode != 0:
                    logger.warning(f"soffice返回非零: returncode={result.returncode}, stderr={result.stderr}")
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"PDF未生成，等待5秒后重试 (第{attempt + 1}/{max_retries}次)...")
                    time.sleep(5)
                    continue
                else:
                ...
                raise RuntimeError(f"soffice转换PDF失败: {e}")
    ...
    def _convert_to_png(
        ...
        try:
            ...
            # 转换PDF为图片
            # 注：poppler_path需要在环境变量中配置或系统PATH中包含
            ...
            if not images:
                raise RuntimeError("pdf2image未能生成图片")
            ...
            import traceback
            traceback.print_exc()
            logger.error(f"pdf2image转换PNG失败: {e}")
            raise
